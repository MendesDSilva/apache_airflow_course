...
def task_group_dag():
    ...
    @task.python
    def process_data(ti):
        api_data = ti.xcom_pull(task_ids='fetch_data_group.fetch_api', key='return_value')
        db_data = ti.xcom_pull(task_ids='fetch_data_group.fetch_db', key='return_value')
        s3_data = ti.xcom_pull(task_ids='fetch_data_group.fetch_s3', key='return_value')
